...
let createApply = async (req, res, next) => {
  try {
    ...
    let createApplyJob = await Apply.create({
      jobs: appliedJobs,
...
let getApplicantsForJob = async (req, res, next) => {
  try {
    ...
    const applicants = await Apply.find({ "jobs.job_id": jobId })
      ...
      .populate(
        "profile",
        "name email contact_no address prefered_job degree field_of_study university position organization job_level roles experience skills"
      );
